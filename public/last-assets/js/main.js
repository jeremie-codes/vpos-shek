...
function initDropzone(selector = ".dropzone", globalOptions = {}) {
    const dropzones = document.querySelectorAll(selector);

    dropzones.forEach((element) => {
        const input = element.querySelector('input[type="file"]');
        const paramName = input?.name || "file";
        const form = element.closest("form");
        const url = form?.getAttribute("action") || '#';
        const isMultiple = input?.hasAttribute("multiple") || false;
        const accept = input?.getAttribute("dz-accept") || 'image/*, application/pdf';

        const defaultOptions = {
            url: url,
            paramName: paramName,
            maxFiles: isMultiple ? 15 : 1,
            maxFilesize: 2,
            acceptedFiles: accept,
            addRemoveLinks: true,
            dictDefaultMessage: "Déposez les fichiers ici ou cliquez pour télécharger",
            headers: {
            ...
            init: function () {
                ...
                this.on("addedfile", function (file) {
                    // Remplacer le champ input par le fichier sélectionné
                ...
                // Si single, remplacer le fichier existant
                if (!isMultiple) {
                ...
                this.on("success", function (file, response) {
                    element.dispatchEvent(new CustomEvent("dropzone:success", {detail: response}));
                });
...
// Vérifier les dropzones requises
const requiredDropzones = document.querySelectorAll('.dropzone input[required]');
requiredDropzones.forEach(function(input) {
    const dropzone = input.closest('.dropzone');
    const errorDiv = dropzone.nextElementSibling;
