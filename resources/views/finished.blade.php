...
<head>
    ...
    <title>{{ __('Paiement clôturé') }} - Vpos</title>
    <!-- Tailwind CSS -->
...
<body class="flex items-center justify-center min-h-screen p-4 bg-slate-50">
    ...
    <div class="relative w-full max-w-md p-8 overflow-hidden text-center bg-white border shadow-xl rounded-2xl">
        ...
        <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 text-3xl border rounded-full shadow-inner bg-slate-100 border-slate-200 text-slate-500">
            <i class="fa-solid fa-circle-check"></i>
        </div>

        <!-- Titre principal -->
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">
            {{ __('Paiement déjà clôturé') }}
        </h1>

        <!-- Message d'explication textuel -->
        <p class="px-2 mt-3 text-sm leading-relaxed text-slate-500">
            {{ __("Votre paiement a déjà été clôturé. Si vous pensez qu'il s'agit d'une erreur, veuillez contacter notre support pour une assistance rapide.") }}
        </p>
